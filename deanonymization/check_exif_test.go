...
var expected_exif = map[string]string{
	"ColorSpace":              "65535",
...
func CompareMaps(t *testing.T, foundTags map[string]string, expectedTags map[string]string) {
	...
	for _, tag := range allTags {
		...
		if ok1 != ok2 {
			if ok1 {
				t.Error(fmt.Sprintf("Mismatch: tag %s found but not expected", tag))
			} else {
				t.Error(fmt.Sprintf("Mismatch: tag %s expected but not found", tag))
			}
		} else if val1 != val2 {
			t.Error(fmt.Sprintf("Mismatch: tag %s found %s expected %s", tag, val1, val2))
		}
	}
}
...
func TestCheckExif(t *testing.T) {
	...
	if len(ctx.report.ExifImages) > 0 {
		t.Error(fmt.Sprintf("Nothing crawled: Should not have detected EXIF"))
	}

	// Test 2: corrupted JPEG image
	ctx.CreateBinaryPage("/test.jpg", 200, "image/jpeg", truncated_jpeg)

	CheckExif(ctx.osreport, ctx.report, ctx.osc)

	if len(ctx.report.ExifImages) > 0 {
		t.Error(fmt.Sprintf("Corrupt image: Should not have detected EXIF"))
	}

	// Test 2: add real JPEG image
	data, err := ioutil.ReadFile("testdata/f1-exif.jpg")
	if err != nil {
		t.Error(fmt.Sprintf("Error reading test image"))
	}
	...
	if len(ctx.report.ExifImages) != 1 {
		t.Error(fmt.Sprintf("Should have detected EXIF in test image"))
	}
	if !strings.HasSuffix(ctx.report.ExifImages[0].Location, "/test2.jpg") {
		t.Error(fmt.Sprintf("Location on EXIF image doesn't match expected"))
	}
	var foundTags = TagsToMap(ctx.report.ExifImages[0].ExifTags)
	CompareMaps(t, foundTags, expected_exif)
}
